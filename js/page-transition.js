var PageTransitions = function(a) {
    ...
    function b() {
        function l() { n.removeClass('fadeOutLeft'), n.show(), n.addClass('fadeInLeft') }
        var m = location.hash.substr(1),
            n = a('#page-ajax-loaded'),
            o = a('#portfolio_grid figure a').each(function() { if (o = a(this).attr('href'), m == 'portfolio/' + o.substr(0, o.length - 5)) { var p = a(this).attr('href'); return l(), n.load(p), !1 } });
        a('.subpages .ajax-page-load').click(function() { a(this).attr('href'); return window.location.hash = 'portfolio/' + a(this).attr('href').substr(0, a(this).attr('href').length - 5), !1 })
    }

    function d(l) { return '' === location.hash ? l ? l : '#' + a('.pt-page-current').attr('data-id') : location.hash.split('/')[0] }
    ...
    function e(l) {
        ...
        if (n = parseInt(l.data('goto')), w >= n) {
            ...
            if (-1 == n ? t < w - 1 ? ++t : t = 0 : -2 == n ? 0 === t ? t = w - 1 : t <= w - 1 ? --t : t = 0 : t = n - 1, u != t) {
                ...
                z.addClass(p).on(j, function() { z.off(j), x = !0, y && f(s, A, z) }), A.addClass(o).on(j, function() { A.off(j), y = !0, x && f(s, A, z) })
            } else s.data('isAnimating', !1)
        } else alert('Transition.js : Invalid \'data-goto\' attribute configuration.');
        k || f(z, A)
    }

    function f(l, m, n) { g(m, n), l.data('isAnimating', !1) }

    function g(l, m) { m.attr('class', m.data('originalClassList')), l.attr('class', l.data('originalClassList') + ' pt-page-current') }
    var j = { WebkitAnimation: 'webkitAnimationEnd', OAnimation: 'oAnimationEnd', msAnimation: 'MSAnimationEnd', animation: 'animationend' }[Modernizr.prefixed('animation')],
    ...
    return {
        init: function(l) {
            a('.pt-page').each(function() {
                ...
                var q = a(this);
                e(q), a(m + ' li').removeClass('active'), a(this.parentNode).addClass('active'), a('.pt-wrapper').animate({ scrollTop: 0 }, 300)
            }), window.onhashchange = function() {
                if (location.hash) {
                    a(m + ' li').removeClass('active');
                    var r = a(m + ' a[href*="' + location.hash.split('/')[0] + '"]'),
                        s = r[0];
                    s = a(s.parentNode), s.addClass('active'), e(r), a('.pt-wrapper').animate({ scrollTop: 0 }, 300), a('#page-ajax-loaded').addClass('fadeOutLeft'), a('#page-ajax-loaded > div').detach(), b()
                }
            };
            var m = l.menu,
                n = d();
            ...
            a('#page').append('<div id="page-ajax-loaded" class="page-ajax-loaded animated fadeInLeft"></div>'), b(), a(document).on('click', '#portfolio-close-button', function() { a('#page-ajax-loaded').addClass('fadeOutLeft'), a('#page-ajax-loaded > div').detach() })
        }
    }
}(jQuery);
